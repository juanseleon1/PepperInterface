...
class Robot(threading.Thread):
    def __init__(self, app, session, gui):
        ...
        if app:
            ...
            self.alListeningMovement.setEnabled(False)
        ...
        self.__modules = {
            # ActivityServices-------------------------------------------------------
            "RUNANIMATION": [self.run_animation, False, "act", True],  # funcionando
            "DEACTIVATEALL": [self.deactivate_all, True, "act", True],  # funcionando
            "ACTIVATEALL": [self.activate_all, True, "act", True],  # funcionando
            ...
            "ACTIVATEFA CEDETEC": [self.activate_face_detection, True, "rob", False],  #
            ...
            "SHOWIMG": [self.show_image, False, "act", True],
            "HIDEIMG": [self.hide_image, False, "act", False],
            "SETTABLETBRIGHT": [self.set_tablet_bright, True, "act", False],
            "SETTABLETVOL": [self.set_tablet_volume, True, "act", False],
            # VoiceServices-------------------------------------------------------
            "SAY": [self.say, False, "act", True],
            "STOPALL": [self.stop_all, False, "act", False],
            ...
            "KILLALL": [self.kill_all, False, "int", False],
        }
        
        
    def addAckForActivity(self, activity, ack):
        self.activities[activity] = ack
    ...
    def run_animation(self, params):
        ...
        try:
            ...
            if params.get("animation") == "GiRARTRONCO":
    ...
    def play_animation(self, animation_names, animation_times, animation_keys):
        try:
            self.alMotion.angleInterpolation(animation_names, animation_keys,
                                                self.change_speed(self.factorVelocity,
    ...
    def get_emotion_state(self):
        emotions = self.gui.get_emotions()
        expressions = {
            "calm": {"value": emotions["calm"], "confidence": 1.0},
            "anger": {"value": emotions["anger"], "confidence": 1.0},
            "joy": {"value": emotions["joy"], "confidence": 1.0},
            "sorrow": {"value": emotions["sorrow"], "confidence": 1.0},
            "laughter": {"value": emotions["laughter"], "confidence": 1.0},
            "excitement": {"value": emotions["excitement"], "confidence": 1.0},
            "surprise": {"value": emotions["surprise"], "confidence": 1.0}
        }
        PersonData = {
            "attention": {"value": emotions["attention"], "confidence": 1.0},
    ...
    def set_leds_intensity(self, sensor, intensity):
        self.alLedsProxy.setIntensity(sensor, intensity)
    ...
    def show_image(self, params):
        self.alTabletService.showImage(params.get("image"))
        ack = -1
        if activities_running.has_key("showImage"):
            ack = activities_running.pop("showImage")
        send(ack, "INTERFACEEVENT","showImage",{})

    # Hide image currently displayed.
    def hide_image(self, params):
        self.alTabletService.hideImage()
        if activities_running.has_key("hideImage"):
            ack = activities_running.pop("hideImage")
            send(ack, "INTERFACEEVENT","hideImage",{})
    ...
    def say(self, params):
        if self.app:
            if "style" in params:
                ...
                self.alAnimatedSpeech.say(params.get("content"), configuration)
            else:
                self.alTexToSpeech.say(params.get("content"))
        else:
            #get random number between 1 and 5
            num = random.randint(1, 5)
            threading.Timer(num, lambda:self.sensorsModule.speechTextDone(True)).start()

    #   This method stops the current and all the pending tasks immediately.
    def stop_all(self, params):
        self.session.service("ALTextToSpeech").stopAll()
        print "STOPALL ACTIVATED AND RAN"
    ...
    def timer_currentState(self):
        ...
        send(-1,"EMOTIONEXTRACTOR", "getUserEmotions", json_params)
        threading.Timer(5.0, self.timer_currentState).start()

    def timer_Battery(self):
        json_params = {"batteryPerc": self.alBatteryProxy.getBatteryCharge()}
        send(-1,"ROBOT_RESOURCES", "battery", json_params)
    ...
    def timer_request_finish_anim(self):
        ...
        if not self.app:
            ...
            my_list = self.alMotion.getTaskList()
        print "my_list", my_list
        my_dict = {item[0]: item[1] for item in my_list}
        print "DICT", my_dict
        
        isActive  = "angleInterpolation" in my_dict or "angleInterpolationBezier" in my_dict
        if activities_running.has_key("runAnimationResponse") and not isActive:
            ack = activities_running.pop("runAnimationResponse")
            send(ack,"MOVEMENT","runAnimationResponse", {"animationEnded":True})
        threading.Timer(1.0, self.timer_request_finish_anim).start()
    ...
    def kill_all(self, params):
        ...
        ack = -1
        if activities_running.has_key("stopAnimationResponse"):
            ack = activities_running.pop("stopAnimationResponse")
            send(ack,"MOVEMENT", "killAll", {})
        
    def setCurrentContext(self, context):
        self.currentContext = context
        Topics.change_context(self.alDialogProxy,context)
