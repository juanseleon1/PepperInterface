...
class pepperModuleV2(object):
    ...
    def activateTopic(self, event_name):
        ...
        send(-1,"placeholder", "SPEECHENGINE", json_params, False)
    ...
    def endOfAnimatedSpeech(self, value):
        if activities_running.has_key("talk") and value:
            json_params = {}
        #    print "it was talkResponse"
        #    ack = activities_running.pop("talk")
        #    json_params["robotIsTalking"] = not value
        #    send(ack,"SPEECHENGINE","talk",  json_params, False)

    # Raised when the person tracked can no longer be found for some time.
    def humanLost(self, value):
        json_params = {}
        json_params["humanLost"] = True
        send(-1,"placeholder", "RAWVIDEO", json_params)
    ...
    def moveFailed(self, value):
        ...
        send(-1,"MOVEMENT","placeholder",  json_params, False)
    ...
    def disabledDevicesChanged(self, value):
        ...
        send(-1,"placeholder", "INTERFACEEVENT", json_params, False)
    ...
    #     send(-1,"placeholder", "int", json_params, False)
    ...
    def tabletVideoFinished(self):
        ...
        if activities_running.has_key("showVideo"):
            print "it was tabletVideoFinished"
            ack = activities_running.pop("showVideo")
            print "ack", ack
        send(-1, "INTERFACEEVENT", "showVideo",  json_params, False)
    ...
    def voiceEmotionRecognized(self, value):
        ...
        send(-1,"placeholder", "SENTIMENTANALYSIS", json_params, False)
    ...
    def personMovedAway(self, value):
        ...
        send(-1,"EMOTIONEXTRACTOR","placeholder",  json_params)

    def personApproached(self, value):
        json_params = {}
        # The value is the ID of the person
        json_params["personApproached"] = True
        send(-1,"placeholder", "EMOTIONEXTRACTOR", json_params)
    ...
    #    send(-1,"placeholder", "EMOTIONEXTRACTOR", json_params, False)
    ...
    def distanceOfTrackedHuman(self, value):
        ...
        if (value is not -1):
            ...
            send(-1, "ROBOT_RESOURCES","placeholder", json_params)
